...
```bash
# Option 1: Using gnome-extensions tool
...
# Then:
# 1. Log out and log back in (Wayland restart)
# 2. gnome-extensions enable spotlight@ninx
# 3. Press Ctrl+Space
```
...
| Check | Tool | Result |
|---|---|---|
| **Shexli** (official e.g.o review tool) | 0 errors, 0 warnings | PASS |
| **GJS SpiderMonkey** runtime | 9/9 checks (enable → open → close → disable) | PASS |
| **Extended search priority tests** | 10/10 (apps first, web last) | PASS |
