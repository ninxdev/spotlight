    ...
    _runSearch(text) {
      ...
      if (calcResult !== null) {
        otherResults.push({
          ...
          activate: () => {
            // CLIPBOARD ACCESS: Only triggered by explicit user action
            // (pressing Enter on the calculator result). This copies the
            // computed number to the clipboard so the user can paste it.
            // No clipboard data is READ or TRANSMITTED — only written,
            // and only when the user explicitly selects the calculator result.
...
export default class SpotlightExtension extends Extension {
  enable() {
    ...
    try {
      ...
      // Use KeybindingManager (icdman pattern).
      // Grab immediately — no delay. The 2-second delay was causing issues
      // when the extension was enabled after GNOME Shell startup.
      ...
      // Grab immediately
      _diag(`Grabbing shortcut: "${shortcutToGrab}"`);
